package my.andr.memorize.layout;
...
public class AutoLinearLayout extends ViewGroup {
	...
	public static class LayoutParams extends ViewGroup.LayoutParams {
		...
		public LayoutParams(int hSpacing, int vSpacing) {
			super(0, 0);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (child.getVisibility() != GONE) {
				...
				line_height = Math.max(line_height, child.getMeasuredHeight()+ lp.vSpacing);
				if (xpos + childw > width) {
					xpos = getPaddingLeft();
					ypos += line_height;
				}
				xpos += childw + lp.hSpacing;
			}
		}
