...
public class EngDbAdapter {
   ...
   private EngDbAdapter(Context ctx) {
        this.mCtx = ctx;
        mDbHelper = new ___________________(mCtx);
        mDb = mDbHelper.___________________();
    }
   public static EngDbAdapter getInstance(Context ctx){
    ...
    public  Cursor  getQuery(int index){
		...
		return mDb._____(DBConstants.TABLE_NAME,null," NO BETWEEN ? AND ? ORDER BY NO ASC ",sqlArgs,null,null,null);
    }
   public SentenceVO getSentence(String item_no) {
		...
		Cursor cursor=mDb._____(DBConstants.TABLE_NAME, fields, " no = " + item_no, null, 
						null, null,null);
